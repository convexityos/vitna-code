...
3. **Several surfaces marked Verified have no tests at all**: `apps/vitna-cli`
   ...
   the new one does and what is asserted. **`apps/vitna-cli` got its first
   tests on 2026-09-24**, for `vitna serve`, which had printed that the daemon
   was listening while binding nothing. Its row says what is still untested.
...
| **Phase 1** | Durable Vertical Slice (CLI, TUI, Daemon, Guarded Runner, Local Receipt v0) | **PARTIAL** | The slice runs end to end, but of the CLI only `vitna serve` is tested, and the daemon's row predates the wire tests #18 and #20 added. The TUI reads receipts and reaches the daemon, and starts no turn, because the daemon serves no `SubmitTurn`. See `docs/PHASE_1_GATE_REPORT.md` |
...
| Local Daemon & Session Store | `crates/daemon/src/server.rs` | One test covers this crate: `test_daemon_server_session_and_run`, a happy-path session and run. The event store beneath it is separately and properly tested (`crates/store`, hash chain and replay). The daemon's own behaviour under a dropped connection, a concurrent session or a restart is not asserted. **Not re-audited since the wire landed:** #18 and #20 added `tests/binary_serves.rs`, `tests/ipc_subscription.rs` and `tests/turns_in_a_session.rs`, and 2026-09-24 added `launch.rs`'s two tests of where the daemon listens and keeps its journal, so "one test" is now an undercount and this row's status has not been re-decided against them. | Smoke tested |
| Non-Interactive CLI Suite | `apps/vitna-cli/src/main.rs` | **Only `vitna serve` is tested.** It printed "Vitna daemon listening on local session channel" while binding nothing; since 2026-09-24 it starts the same daemon as `vitna-coded`, through `vitna_daemon::launch`. `tests/serve_listens.rs` runs the built `vitna` binary and asserts that it binds the declared endpoint and completes the handshake, keeps its journal where `--store` (or the older `--db`) says, and names the failure on stderr and exits non-zero when it cannot start. The handshake assertion was mutation checked against the old behaviour. **No other subcommand has a test**: nothing invokes `run`, `sessions`, `verify` or `doctor`, checks their exit codes, or covers argument parsing, and `resume`, `review`, `apply` and `receipt show`/`export` print that they ran without doing anything. | Partial |
